...
class path(nn.Module):
    ...
    def forward(self, x):
        states = torch.zeros((self.steps, 6)).to(device)
...
if __name__ == "__main__":
    ...
    robot = Robot(cfg)
    planner = Planner(torch.zeros(6).to(device), torch.zeros(6).to(device), conf, robot)
    fps = 10
    ...
    plt.imshow(np.hstack([cv2.normalize(depth.unsqueeze(-1).repeat(1,1,3).to(device).detach().cpu().numpy(),dst=None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX), color.to(device).detach().cpu().numpy()]))
    plt.xticks([])
    plt.yticks([])
    plt.savefig('n_0.png', bbox_inches='tight', pad_inches=0.0)
    ...
    for frame in range(100):
        ...
        if k in {65362, 65364, 65361, 65363}:  # up, down, left, right
            ...
            plt.xticks([])
            plt.yticks([])
            plt.savefig('n_{}.png'.format(frame+1), bbox_inches='tight', pad_inches=0.0)
            #cv2.imshow('Safety Filter', np.hstack([cv2.normalize(depth.unsqueeze(-1).repeat(1,1,3).to(device).detach().cpu().numpy(),
            #dst=None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX), color[:, :, [2,1,0]].to(device).detach().cpu().numpy()]))
